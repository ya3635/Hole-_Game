...
namespace PaintIn3D
{
	...
	[ExecuteInEditMode]
	[HelpURL(CwCommon.HelpUrlPrefix + "CwHitBetween")]
	[AddComponentMenu(PaintCore.CwCommon.ComponentHitMenuPrefix + "Hit Between")]
	public class CwHitBetween : MonoBehaviour
	{
		...
		/// ThisRotation = The current <b>TileTransform.rotation</b> will be used.
		/// ThisLocalRotation = The current <b>TileTransform.localRotation</b> will be used.
		...
		/// <summary>If you want to display something at the hit point (e.g. particles), you can specify the TileTransform here.</summary>
		...
		public CwPointConnector Connector { get { if (connector == null) connector = new CwPointConnector(); return connector; } } [SerializeField] private CwPointConnector connector;
		...
		private void SubmitHit(bool preview)
		{
			if (pointA != null && pointB != null)
			{
				...
				if (Physics.Raycast(ray, out hit3D, maxDistance, layers) == true && (hit2D.collider == null || hit3D.distance < hit2D.distance))
				{
					...
					{
						connector.HitCache.InvokeCoord(gameObject, preview, priority, pressure, new CwHit(hit3D), finalRotation);
					}
					else if (emit == EmitType.TrianglesIn3D)
					{
						connector.HitCache.InvokeTriangle(gameObject, preview, priority, pressure, new CwHit(hit3D), finalRotation);
					}
				}
				// Hit 2D?
		...
		private void CalcHitData(Vector3 hitPoint, Vector3 hitNormal, Ray ray, out Vector3 finalPosition, out Quaternion finalRotation)
		{
			...
			switch (orientation)
			{
				...
				{
					var finalUp     = PaintCore.CwCommon.GetCameraUp(_camera);
					var finalNormal = normal == NormalType.HitNormal ? hitNormal : -ray.direction;
...
{
	using UnityEditor;
	using TARGET = CwHitBetween;

	[CanEditMultipleObjects]
	[CustomEditor(typeof(TARGET))]
	public class CwHitBetween_Editor : CwEditor
	{
		protected override void OnInspector()
		{
			...
			BeginIndent();
				if (Any(tgts, t => t.Orientation == CwHitBetween.OrientationType.CameraUp))
				{
					Draw("_camera", "Orient to a specific camera?\nNone = MainCamera.");
				}
				if (Any(tgts, t => t.Orientation == CwHitBetween.OrientationType.WorldUp || t.Orientation == CwHitBetween.OrientationType.CameraUp))
				{
					Draw("normal", "Which normal should the hit point rotation be based on?");
				}
				if (Any(tgts, t => t.Orientation == CwHitBetween.OrientationType.CustomRotation || t.Orientation == CwHitBetween.OrientationType.CustomLocalRotation))
				{
			...
			if (DrawFoldout("Advanced", "Show advanced settings?") == true)
			{
				BeginIndent();
					...
					Draw("point", "If you want to display something at the hit point (e.g. particles), you can specify the TileTransform here.");
			...
			var point    = tgt.Draw == CwHitBetween.EmitType.PointsIn3D;
			var line     = tgt.Draw == CwHitBetween.EmitType.PointsIn3D && tgt.Connector.ConnectHits == true;
			var triangle = tgt.Draw == CwHitBetween.EmitType.TrianglesIn3D;
			var coord    = tgt.Draw == CwHitBetween.EmitType.PointsOnUV;
